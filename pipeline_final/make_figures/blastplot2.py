...
def make_plotf(n):
    ...
    mn=0
    inters=[x for x in os.listdir("blast_round_two_data") if n in x]
    for m in inters:
        if "Meta" in m:
            df=pd.read_csv(f"blast_round_two_data/{m}",sep="\t",dtype={"staxids": str})
        else:
            df=pd.read_csv(f"blast_round_two_data/{m}",sep="\t",dtype={"staxids": str})
        ...
        for index, row in d.iterrows():
            ax[0].hlines(np.log10(float(row["evalue"])+floor),xstart+float(row["qstart"]),xstart+float(row["qend"]),color=row["color"],linewidth=1)
        mn=min(mn,ax[0].get_ylim()[0]-.10*(ax[0].get_ylim()[1]-ax[1].get_ylim()[0]))
    ints=results2[n]
    for y in ints:
        if ints[y]=="HGT":
            color="purple"
    ...
    ax[0].set_xlabel("Position with respect to query (N-C)")
    ...
    for m in inters:
        ...
        mn=min(mn,ax[1].get_ylim()[0]-.10*(ax[1].get_ylim()[1]-ax[1].get_ylim()[0]))
    ints=results2[n]
    for y in ints:
        if ints[y]=="HGT":
            color="purple"
    ...
    plt.savefig(f"blast_result_plots/{n}/round2_plot.svg",format="svg", bbox_inches="tight")
